-- Genel Başvuru (Khilonfast Bilgi Formu) — CRM liste + public form seed
-- Çalıştırma (Türkçe karakter için charset ZORUNLU):
--   mysql --default-character-set=utf8mb4 -u <user> -p <db> < genel_basvuru_form.sql
-- İdempotent + oturum değişkenine bağımsız (list_id slug alt-sorgusuyla kesinlenir).

-- 1) Genel başvuranların düşeceği ayrı liste
INSERT INTO crm_lists (slug, name, description, type)
VALUES ('genel-basvurular', 'Genel Başvurular',
        'Khilonfast bilgi formu (genel başvuru landing page) kayıtları', 'static')
ON DUPLICATE KEY UPDATE name = VALUES(name), description = VALUES(description);

-- 2) Public form: LP → /api/crm-public/form/genel-basvuru/submit
INSERT INTO crm_forms (slug, name, description, fields_json, actions_json, success_message, double_opt_in, is_active)
VALUES (
    'genel-basvuru',
    'Genel Başvuru (Bilgi Formu)',
    'Khilonfast bilgi formu — genel başvuru landing page',
    JSON_ARRAY(
